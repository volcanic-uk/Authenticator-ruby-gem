...
RSpec.describe Volcanic::Authenticator::V1::Token, :vcr do
  ...
  describe 'Validate by service' do
    context 'When missing token key' do
      ...
      it { expect(token.new('wrong-id').validate_by_service).to be false }
    end

    context 'When expire token key' do
      it { expect(token.new(mock_expired_token_key).validate_by_service).to be false }
    end

    context 'When invalid signature' do
      it { expect(token.new(mock_invalid_token_key).validate_by_service).to be false }
    end

    context 'When token is valid' do
      it { expect(token.new(mock_token_key).validate_by_service).to be false }
    end
  end

  describe 'Decodes and fetch claims' do
    context 'When invalid token key' do
      it { expect { token.new.decode_and_fetch_claims }.to raise_error token_error }
      it { expect { token.new('wrong-token').decode_and_fetch_claims }.to raise_error token_error }
    end

    context 'When success' do
      subject { token.new(mock_token_key).decode_and_fetch_claims }
