...
RSpec.describe Volcanic::Authenticator do
  ...
  describe 'Permission' do
    ...
    describe 'Create' do
      ...
      context 'When duplicate name' do
        let(:duplicate_name) { SecureRandom.hex 6 }
        before { permission.create(duplicate_name, new_identity.id) }
        it { expect { permission.create(duplicate_name, new_identity.id) }.to raise_error Volcanic::Authenticator::PermissionError }
      end

      context 'When invalid creator_id' do
        it { expect { permission.create(mock_name, mock_name) }.to raise_error Volcanic::Authenticator::PermissionError }
      end

      context 'with name and creator_id' do
        subject { permission.create(mock_name, new_identity.id) }
        its(:name) { should_not be nil }
        its(:id) { should_not be nil }
      ...
      context 'with description' do
        subject { permission.create(mock_name, new_identity.id, 'descriptions') }
    ...
    describe 'Retrieve' do
      context 'When retrieve all' do
        subject { permission.retrieve }
        it { should be_an_instance_of(Array) }
      end

      context 'When missing id' do
        it { expect { permission.retrieve(nil) }.to raise_error Volcanic::Authenticator::ConnectionError }
        it { expect { permission.retrieve('') }.to raise_error Volcanic::Authenticator::ConnectionError }
      end

      context 'When invalid id' do
        it { expect { permission.retrieve('wrong_id') }.to raise_error Volcanic::Authenticator::PermissionError }
      end

      context 'When retrieve by id' do
        subject { permission.retrieve(new_permission.id) }
        its(:name) { should_not be nil }
        its(:id) { should_not be nil }
        its(:creator_id) { should_not be nil }
